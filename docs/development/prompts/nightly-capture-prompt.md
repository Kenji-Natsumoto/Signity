...
Routine（cron `0 16 * * *` = JST 01:00 毎日）に登録する無人実行プロンプトの正本です。
**実行は 01:00、対象は前日**です。実行日と対象日が 1 日ずれます。
...
Kenji-Natsumoto/Signity を clone し（セッションに無ければ add_repo で追加してから clone する）、
...
【スキャン窓】D 00:00:00+09:00 〜 D+1 00:00:00+09:00（対象日の丸一日）。
docs/journal/YYYY/MM/D.scan.json が既にあれば D は処理済み。既存の記述は書き換えず、
差分だけを末尾に「再スキャン（実行時刻）」として追記する。

【Phase 1 収集】次を横断して対象日 D の差分を取る。取れなかったソースは必ず記録に残す。
 - Slack: slack_list_user_channels で参加チャンネルと DM を列挙し、slack_read_channel で D の分を読む
 - Gmail: D の受信・送信。ラベル Signity/Inbox を優先
 - Google Calendar: D に実施された会議
 - Google Drive: D に更新されたファイル（議事録・設計文書）
 - GitHub: スコープ内リポジトリの commit / PR / issue
 - docs/journal/inbox/D/ : Mac 側コレクタ（00:50 JST 実行）と手動投下の素材
...
横断で「返していない球（自分宛の依頼・質問で D 中に返答がないもの）」と
「既存 Decision Event の review_due_at の接近」も出す。
別ソースで食い違う発言は、直さずに矛盾として報告する。

【Phase 4 記録】
 - ./scripts/new-journal.sh --yesterday --nightly で docs/journal/YYYY/MM/D.md を用意し、
   templates/nightly-scan-section.md の構成で埋める。
   人間が D の当日中に書いた記述は一切書き換えない。末尾に追記するだけ。
 - docs/journal/YYYY/MM/D.scan.json を書く（窓・ソース別の取得可否・件数）。
   取得できなかったソースは status: unavailable として必ず列挙する。省略しない。
 - (a) のうち Decision Event の条件（docs/README.md 4 章）を満たすものだけ、
   ledger/pending/DE-<Dの YYYYMMDD>-NNN.json に Draft を作る。
   NNN は対象日の連番。既存 pending の最大値 +1。ledger/pending/DE-20260728-001.json を雛形にする。
...
- cron: `0 16 * * *`（UTC。JST 01:00 毎日。対象は前日）
- 新規セッションで実行: 有効
